...
function Row({fetchUrl, category}) {
    ...
    const scrollMoviesRight = (() => {
        ...
        if (checkAllScrolled(e)) setAllScrolled(true)
    ...
    return (
        <div className="row">
            ...
            <div className="externalContainer" onMouseEnter={onMouseEnter} onMouseLeave={onMouseOut}>

                <div ref={movieRow} className="rowContainer">
                    {movies.map((movie) => {
                        return <img key={movie.id} id={movie.id} className="moviePoster" src={requests.imageBase + movie.poster_path} alt=""></img>
                    })}
                </div>
